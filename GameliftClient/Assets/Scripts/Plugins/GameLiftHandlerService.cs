...
public class GameLiftHandlerService : MonoBehaviour
{
    ...
    private static int GetAvailablePort()
    {
        ...
        var port = UnityEngine.Random.Range(7000, 8000);
        ...
        while (i < maxAttempts)
        {
            ...
            {
                Debug.Log("Port used");
                port = UnityEngine.Random.Range(7000, 8000);
            }
            else
            { 
    ...
    public GameSession MySession { get; private set; }

    //void Start()
    //{
    //    GetComponent<GameServerPortal>().StartServer(MapEnvironment.Asteria, "127.0.0.1", 7777);
    //}
    ...
    void Start()
    {
        ...
        Debug.Log("Available Port: " + _serverPort);

        if (GameLiftClientHandler.IsLocal)
        {
            LocalGameSessionComplete();
            return;
        }
        ...
        if (initSDKOutcome.Success)
        {
            ProcessParameters processParameters = new ProcessParameters(
               ...
               this.OnHealthCheck,
               _serverPort, // This game server tells GameLift the port it will listen on for incoming player connections.
               new LogParameters(new List<string>()
               {
               // Here, the game server tells GameLift what set of files to upload when the game session ends.
               // GameLift will upload everything specified here for the developers to fetch later.               
               ServerLoggingFile.Logfilename
               }
            ));
    ...
    void OnGameSession(GameSession gameSession)
    {
        ...
        //Game Session Complete, Now you can start the server based on the game session data

        //Start the client checking, to make sure there's client connected to this serverr. To prevent ghost session                

        GameLiftServerAPI.ActivateGameSession();
    }

    void LocalGameSessionComplete()
    {
    ...
    public static void RemovePlayerSession(string playerSessionId)
    {
    ...
    private static void FinalizeServerProcessShutdown()
    {
